...
include_once 'Model/config.php';
	...
	{
		//consulta
		$insertar="INSERT INTO tbl_pqo(nombre, correo, telefono, comentario, fecha) values (?,?,?,?,?)";
		try {
	...
	public function ListaDatos()
	{
		...
		$datocategoria=array();
		// declaramos consulta
		$consulta="SELECT * FROM tbl_pqo ORDER BY id_pqo";
		try {
			...
			foreach ($resultado->fetchAll(PDO::FETCH_OBJ) as $datos) {
				...
				$categoria->__SET('id_pqo',$datos->id_pqo);
	...
	public function buscar($id_pqo)
	{
		// declaramos consulta
		$buscar="SELECT * FROM tbl_pqo where id_pqo=?";
		try {
			//reparamos la consulta con la conexion
	...
	public function eliminar($id_pqo)
	{
		// declaramos consulta
		$borrar="DELETE  FROM tbl_pqo WHERE id_pqo=?";
		try {
			//ejecutamos con la conexion y almacenamos lo que trae en la variable "resultado"
			$this->conexion->prepare($borrar)->execute(array($id_pqo));
			return true;
