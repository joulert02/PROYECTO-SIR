<?php
	...
	class personModel extends mainModel
	{
		...
		protected function agregar_persona_modelo($datos){
			...
			$insertarP = "INSERT INTO tbl_persona (nombres, apellidos, tipo_documento_tipo_documento, documento, telefono, nro_Celular, direccion, ciudad, departamento, tipo_persona_tipo_persona, estado) 
			VALUES (:nombres, :apellidos, :tipoD, :documento, :telefono, :celular, :direccion, :ciudad, :departamento, :tipoP, :estado)";
			$sql= mainModel::conectar()->prepare($insertarP);
			$sql->bindParam(":nombres",$datos['nombres']);
			$sql->bindParam(":apellidos",$datos['apellidos']);
			$sql->bindParam(":tipoD",$datos['tipoD']);
			...
			$sql->bindParam(":tipoP",$datos['tipoP']);
		...
		public function listar()
		{
			...
			$consultar = "SELECT per.estado as numEstado, per.id_persona, per.nombres, per.apellidos, doc.nombre_documento, per.documento, per.telefono, 
			per.nro_Celular, per.direccion, per.ciudad, per.departamento, tper.nombre_tipo,
			CASE WHEN per.estado = 1 THEN 'Activo' ELSE 'Inactivo' END AS estado FROM tbl_persona per
		...
		public function eliminar($id)
		{
			...
			try{
				mainModel::conectar()->prepare($borrar)->execute(array($id));
				return true;
			}catch (Exception $e){
