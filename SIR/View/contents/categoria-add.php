<?php
  ...
  try {
    ...
    if (isset($_REQUEST['nombre']) && isset($_REQUEST['estado']) && !isset($_REQUEST['editando'])) {
    ...
    }elseif (isset($_REQUEST['id'])  && !isset($_REQUEST['editando'])) {
        $id = $_REQUEST['id'];
		...
		try {
			...
			$conn->prepare($borrar)->execute(array($id));
