...
if (isset($_REQUEST['Producto_id_producto']) && isset($_REQUEST['cantidad'])) {
    ...
    $detalles="INSERT INTO tmp_salida (id_producto,cantidad_tmp) values (?,?)";
    try{
    ...
    $json=array();
        $consulta="SELECT id_tmp,t.id_producto,t.cantidad_tmp,i.nombre_producto,i.referencia FROM tmp_salida t JOIN tbl_producto i ON i.id_producto = t.id_producto";
        try {
        ...
        $consulta="DELETE FROM `tmp_salida` WHERE `id_tmp`=$cod";
        try {
            $resultado=$conexion->query($consulta);
            $resultado->execute();
          echo "delete";
