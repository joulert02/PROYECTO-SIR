...
try {
    $conn = new PDO("mysql:host=$servername;dbname=s.i.r;charset=utf8", $username, $password);
...
if (isset($_REQUEST['nombre']) && isset($_REQUEST['estado']) && (!isset($_REQUEST['editando']) || $_REQUEST['editando'] == false)) {
    ...
    $nombreCategoria = $conn->prepare("SELECT * FROM `tbl_categoria_producto` WHERE categoria = '$nombre'");
    $nombreCategoria->execute();
    if ($nombreCategoria->rowCount() >= 1) {
        echo 2;
    } else {
    ...
    $json = array();
    $consulta = "SELECT * FROM tbl_categoria_producto ORDER BY id_Categoria DESC";
    try {
